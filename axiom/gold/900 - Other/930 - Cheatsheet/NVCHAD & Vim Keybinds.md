...
gd (Go to Definition): Springt direkt zur Definition der Klasse, Methode oder Variable unter deinem Cursor (z. B. wenn du auf einer Methode aus SqlConnectionManager stehst).

K (Hover): Öffnet ein kleines Pop-up-Fenster mit der Javadoc und der Methodensignatur unter dem Cursor.

<leader>ra (Rename): Benennt eine Variable oder Methode projektweit um (inklusive aller Referenzen in anderen Dateien). Hinweis: <leader> ist in NvChad standardmäßig die Leertaste.

<leader>ca (Code Action): Öffnet Vorschläge für Quick-Fixes, automatische Imports oder das Generieren von Gettern/Settern und Konstruktoren (dafür haben wir die extendedClientCapabilities in deinem Log gesehen).

[d / ]d: Springt zum vorherigen oder nächsten Syntaxfehler/Warnung im Code.
